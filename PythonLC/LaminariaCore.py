...
class InteractionUtils:
    ...
    async def getrolenamed(role: str, guild: interactions.Guild, create: bool = False, exact: bool = True):
        ...
        for r in guild.roles:
            ...
            if create is True:
                random_colour = "%02x%02x%02x" % (
                random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))
                return_role = await guild.create_role(name=role, color=int(random_colour, 16))
                return return_role
    ...
    @staticmethod
    async def get_formatted_date_now(include_seconds: bool = False, formatting: int = 1):
        """
        Returns the current date in the handy DD/MM/YY - HH:MM:SS format (default) or in the specified one.
        :param formatting: Format type -> int
        :param include_seconds: If set to True, include seconds in the format.
    ...
    @staticmethod
    async def get_formatted_date(date: datetime, include_seconds: bool = False):
        """
        Returns a given date in the handy DD/MM/YY - HH:MM:SS format.
        :param date: The date to be formatted -> datetime.datetime
        :param include_seconds: If set to True, include seconds in the format.
    ...
    async def send_loading(self, channel_id: int, colour: int = int("eb4034", 16)):
        """
    ...
    async def show_help_menu(self, ctx: interactions.CommandContext, colour: int = int("eb4034", 16),
        ...
        for category in sorted(commands_dictionary):
            ...
            for command in sorted(commands_dictionary[category]):
                ...
                category_embed.add_field(name=name.title(),
    ...
    async def convert_txt_to_pdf(self, path: str):
        """
    ...
    async def load_missing_perms_embed(self, colour=discord.Colour.red()):
        """
    ...
    async def interactive_dialog(self, **kwargs):
        """
